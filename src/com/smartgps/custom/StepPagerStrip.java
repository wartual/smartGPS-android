...
public class StepPagerStrip extends View {
	...
	public StepPagerStrip(Context context, AttributeSet attrs, int defStyle) {
		...
		if (density == DisplayMetrics.DENSITY_LOW) {
			mTabWidth = width / 4 + 10;
		} else {
			mTabWidth = width / 4 + 38;
		}
		...
		mPrevTabPaint.setColor(res
				.getColor(R.color.step_pager_previous_tab_color));
		...
		mSelectedLastTabPaint.setColor(res
				.getColor(R.color.step_pager_selected_last_tab_color));
		...
		for (int i = 0; i < mPageCount; i++) {
			...
			canvas.drawRect(mTempRectF, i < mCurrentPage ? mPrevTabPaint
					: (i > mCurrentPage ? mNextTabPaint
							: (i == mPageCount - 1 ? mSelectedLastTabPaint
									: mSelectedTabPaint)));
		}
	}
